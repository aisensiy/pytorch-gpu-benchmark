...
parser.add_argument('opertation', type=str, choices=[
...
parser.add_argument('--NUM_TEST', '-n', type=int, default=50,
                    required=False, help="Num of Test")
...
def train(type='single'):
    ...
    for model_type in MODEL_LIST.keys():
        for model_name in MODEL_LIST[model_type]:
            ...
            print(model_name, ' model average train time : ',
                  sum(durations)/len(durations), 'ms')
            del model
        ...
        for model_type in MODEL_LIST.keys():
            for model_name in MODEL_LIST[model_type]:
                ...
                for step, img in enumerate(rand_loader):
                    ...
                    if step >= args.WARM_UP:
                        durations.append((end - start)*1000)
                print(model_name, ' model average inference time : ',
                      sum(durations)/len(durations), 'ms')
                del model
                benchmark[model_name] = durations
    return benchmark
...
def experiment(env_name, device_name):
    ...
    experiment_result_data_dir = os.path.join(
        RESULT_ROOT_DIR, env_name, device_name, 'data')
    ...
    now = time.localtime()
    start_time = str("%04d/%02d/%02d %02d:%02d:%02d" % (now.tm_year,
    ...
    for precision in precisions:
        ...
        path = ''.join((experiment_result_data_dir, '/', device_name.replace(" ",
                                                                             "_"), "_", precision, '_model_train_benchmark.csv'))
        ...
        path = ''.join((experiment_result_data_dir, '/', device_name.replace(" ",
                                                                             "_"), "_", precision, '_model_inference_benchmark.csv'))
        inference_result_df.to_csv(path, index=False)

        print("save {} succeed !".format(path))

    now = time.localtime()
    end_time = str("%04d/%02d/%02d %02d:%02d:%02d" % (now.tm_year,
                                                      now.tm_mon, now.tm_mday, now.tm_hour, now.tm_min, now.tm_sec))
...
def insert_series_to_df(df, env, gpu, phase, precision, average_time):
    for model, time in average_time.iteritems():
        df = df.append({"envs": env, "gpus": gpu, "models": model, "phases": phase,
                        "precisions": precision, "time": int(time)}, ignore_index=True)
    return df
...
def build_small_data_frame_for_benchmark(env, gpu, benchmark_csv_files_dir):
    ...
    for file in csv_files:
        data = pd.read_csv(os.path.join(
            benchmark_csv_files_dir, file), index_col=False)
        average_time = data.mean()
        if 'train' in file and 'half' in file:
            df = insert_series_to_df(
                df, env, gpu, 'train', 'half', average_time)
        elif 'train' in file and 'float' in file:
            df = insert_series_to_df(
                df, env, gpu, 'train', 'float', average_time)
        elif 'train' in file and 'double' in file:
            df = insert_series_to_df(
                df, env, gpu, 'train', 'double', average_time)
        elif 'inference' in file and 'half' in file:
            df = insert_series_to_df(
                df, env, gpu, 'inference', 'half', average_time)
        elif 'inference' in file and 'float' in file:
            df = insert_series_to_df(
                df, env, gpu, 'inference', 'float', average_time)
        elif 'inference' in file and 'double' in file:
            df = insert_series_to_df(
                df, env, gpu, 'inference', 'double', average_time)
        else:
...
def plot_models_on_same_gpu(models_time_info, gpu, save_image_dir):
    ...
    for phase in phases:
        for precision in precisions:
            ...
            models_time_info.sort_values(
                by=['models'], inplace=True, ascending=False)

            names = models_time_info[(models_time_info.phases == phase) & (
                models_time_info.precisions == precision)]['models'].tolist()
            times = models_time_info[(models_time_info.phases == phase) & (
                models_time_info.precisions == precision)]['time'].tolist()
            ...
            plt_title = '{} {} models with {} precision'.format(
                gpu, phase, precision)
...
def run_models_on_special_gpu():
    if len(args.ENVS) != 1:
        raise Exception("env nums must be one, but current nums is : {}".format(
            len(args.ENVS)), args.ENVS)
    env = args.ENVS[0]
    # nvml.nvmlInit()
    # gpu_count = nvml.nvmlDeviceGetCount()
    gpu_count = torch.cuda.device_count()
    gpu = "".join((device_name.replace(" ", "_"),
                   '_', str(gpu_count), '_gpus'))

    experiment(env, gpu)

    experiment_result = RESULT_ROOT_DIR
    benchmark_csv_files_dir = os.path.join(experiment_result, env, gpu, 'data')
    benchmark_data_frame_gpu_level = build_small_data_frame_for_benchmark(
        env, gpu, benchmark_csv_files_dir)

    save_image_dir = os.path.join(experiment_result, env, gpu, 'images')
    if not os.path.exists(save_image_dir):
        os.makedirs(save_image_dir)
    plot_models_on_same_gpu(
        benchmark_data_frame_gpu_level, gpu, save_image_dir)


def build_gpus_data_frame_benchmark(experiment_result, env, gpus):
    gpus_data_frame = pd.DataFrame(columns=DF_COLUMNS)
    for gpu in gpus:
        benchmark_csv_files_dir = os.path.join(
            experiment_result, env, gpu, 'data')
        small_data_frame = build_small_data_frame_for_benchmark(
            env, gpu, benchmark_csv_files_dir)
        gpus_data_frame = pd.concat(
            [gpus_data_frame, small_data_frame], axis=0, ignore_index=True)
...
def statistic_benchmark_between_gpus():
    ...
    env = args.ENVS[0]
    if len(args.GPUS) < 2:
        raise Exception("gpu nums must more one, but current nums is : {}".format(
            len(args.GPUS)), args.GPUS)
    gpus = args.GPUS
    ...
    gpus_data_frame = build_gpus_data_frame_benchmark(
        experiment_result, env, gpus)
    for phase in phases:
        for precision in precisions:
            plot_image_for_compare_model_benchmark_on_multiple_gpus(
                env, phase, precision, gpus_data_frame)


def validate_args_for_statistic_benchmark_between_envs():
    experiment_result = RESULT_ROOT_DIR
    if len(args.ENVS) < 2:
        raise Exception("env nums must more one, but current nums is : {}".format(
            len(args.ENVS)), args.ENVS)
    envs = args.ENVS
    if len(args.GPUS) != 1:
        raise Exception("gpu nums must be one, but current nums is : {}".format(
            len(args.GPUS)), args.GPUS)
    gpu = args.GPUS[0]
...
def build_envs_data_frame_benchmark(experiment_result, envs, gpu):
    ...
    for env in envs:
        ...
        small_data_frame = build_small_data_frame_for_benchmark(
            env, gpu, benchmark_csv_files_dir)
        envs_data_frame = pd.concat(
            [envs_data_frame, small_data_frame], axis=0, ignore_index=True)
...
def statistic_benchmark_between_envs():
    ...
    envs_data_frame = build_envs_data_frame_benchmark(
        RESULT_ROOT_DIR, envs, gpu)

    for phase in phases:
        for precision in precisions:
            plot_image_with_models_benchmark_on_special_gpu_between_envs(
                gpu, phase, precision, envs_data_frame)
